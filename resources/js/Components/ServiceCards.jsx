    ...
    <>
      <div className="cards" data-stagger>
        {SERVICES.map((s, i) => (
          ...
          >
            <div className={'ic' + (s.image ? ' ic-img' : '')}>
              {s.image ? <img src={s.image} alt={s.title} loading="lazy" /> : <Icon name={s.icon} />}
            </div>
            <h3>{s.title}</h3>
            <p>{s.desc}</p>
            <ul className="feats">
        ...
        {active && (
          <>
            <div className="dlg-head">
              <div className={'dlg-ic' + (active.image ? ' ic-img' : '')}>
                {active.image ? <img src={active.image} alt={active.title} /> : <Icon name={active.icon} />}
              </div>
              <h3>{active.title}</h3>
            </div>
